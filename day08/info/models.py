...
class BaseModel(object):
    """ models base class, add time and updata time for every model"""
    create_time = db.Column(db.DateTime, default=datetime.now)  # record create time
    update_time = db.Column(db.DateTime, default=datetime.now, onupdate=datetime.now)  # record update time


# user collection table, built many-to-many relative of user and news
...
class User(BaseModel, db.Model):
    ...
    nick_name = db.Column(db.String(32), unique=True, nullable=False)  # nickname
    ...
    avatar_url = db.Column(db.String(256))  # path of user Head portrait
    last_login = db.Column(db.DateTime, default=datetime.now)  # login time of last time
    is_admin = db.Column(db.Boolean, default=False)
    signature = db.Column(db.String(512))  # user signature
    gender = db.Column(
        db.Enum(
            "MAN",
            "WOMAN"
    ...
    # all news of current user collection
    collection_news = db.relationship("News", secondary=tb_user_collection, lazy="dynamic")
    # all fans of user, Reverse reference　followed, represent follow who
